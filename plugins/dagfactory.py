...
class DagFactory:
    ...
    def generate_dags(self, globals: Dict[str, Any]) -> None:
        ...
        for dag_name, dag_config in dag_configs.items():
            ...
            """
            tasks: Dict[str, Any] = {}
            for task_instance in dag["dag"].tasks:
                task_id: str = str(task_instance).split(':')[1].replace('>','').strip()
                tasks[task_id] = task_instance

            globals[dag["dag_id"]]: DAG = dag["dag"]
            return dag["dag"], tasks
